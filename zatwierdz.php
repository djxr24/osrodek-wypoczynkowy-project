...
if(isset($_SESSION['zalogowany'])) //jeśli zalogowany...
{
	...
	if ($level == 1 or 2 or 3 or 4 or 5 or 6) //jeśli użytkownik ma odpowiednie uprawnienia
	{
		...
		function walidacja_wartosci($nazwy_wszystkich_tabel, $nazwy_wszystkich_wartosci, $nazwy_tabel, $kolumny, $warunki) // funkcja do skonczenia return walidacja prawda/fałsz i przekaz to do funkcji baza
		{ //TU ZROBIĆ to co skomentowane pod spodem
			...
			foreach ($warunki as $warunek)
			{
				...
				// $warunek, $nazwy_wszystkich_tabel, $nazwy_wszystkich_wartosci
				
				
				$komenda_warunki.=$nazwy_wszystkich_tabel[$warunek]."='".$nazwy_wszystkich_wartosci[$warunek]."'";
		...
		function baza($wartosci, $nazwa_str_do_powrotu, $nazwy_tabel, $nazwy_kolumn, $typ_komendy, $warunki, $wyswietlany_komunikat, $wyświetlany_komunikat_błędu, $walidacja)
		{
			require('config.php'); //
			...
			if ($typ_komendy == 1) {
				...
				if ($walidacja == 0)//(mysql_num_rows(mysql_query...) //TU ROBIĆ
				{
					...
					echo $wyświetlany_komunikat_błędu."<br/><br/>";
				...
				{	
					...
					//echo " Kom: ".$sql."<br>";
					//echo " War: ".$komenda_warunki."<br>";;
					$results=mysql_query($sql) or die ('Wykonanie zapytania nie powodło sie naprawde. Błąd:' .mysql_error()); //zatwierdź komendę lub wyświetl błąd
				...
				{	
					...
					//echo " Kom: ".$sql."<br>";
					//echo " War: ".$komenda_warunki."<br>";
					$results=mysql_query($sql) or die ('Wykonanie zapytania nie powodło sie. Błąd:' .mysql_error()); //zatwierdź komendę lub wyświetl błąd
	...
	// $walidacja_warunki = array('Pole1' => 'Warunek1'
		...
		//function baza($wartosci, $nazwa_str_do_powrotu, $nazwa_tabeli, $nazwy_kolumn, $typ_komendy, $where_warunek, $wyswietlany_komunikat, $walidacja)
			...
			if (count(($_POST['wartosci'])) < count(($_POST['kolumna'])))
			{
			...
			// echo "Nazwa 2: ".$db_name." ";
			
			//require_once('config.php'); //wystartuj sesję
			
			baza ($_POST['wartosci'], $_POST['strona_wroc'], $_POST['tabela'], $_POST['kolumna'], $_POST['typ_komendy'], $_POST['warunek'], $_POST['komunikat_powodzenie'], $_POST['komunikat_niepodzenie'], $walidacja); //tak musi byc z post
			//baza ($post, "domki.php", "domek", $nazwy_kolumn, 1, "Tu jest komunikat", $where_warunek, 1);
			
			// echo "Nazwa 4: ".$db_name." ";
			}
		}
		else
		{
