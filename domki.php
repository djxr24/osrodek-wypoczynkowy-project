	...
	if ($level == 6 || $level == 5 || $level == 4 || $level == 2 ) // sprawdź uprawnienia
	{
		...
		elseif (isset($_POST['wyszukaj'])) //jeśli klinięto wyszukaj
		{
			...
			else // ...a jeśli zostało
			{
				...
				{
					echo "<div style ='width: 650px; height: 100px; top: 50px; left: 15px; position: absolute; border: 1px; border-style: none; border-color: #000000; overflow-y: auto; overflow-x: hidden'>";
					echo "<table border=1><tr>";
					echo "<td><b>Nazwa</b></td>";
					echo "<td><b>Opis</b></td>";
					echo "<td><b>Cena</b></td>";
		...
		if (isset($_POST['dodaj'])) // jeśli klinieto 'dodaj'
		{
			...
			//Array ( [wartosci] => Array ( [0] => aaaaaaa [1] => aaaaaaa [2] => aaaaaaa ) [walidacja_tabela] => Array ( [0] => domek ) [walidacja_kolumna] => Array ( [0] => Nazwa ) [walidacja_warunek] => Array ( [0] => Nazwa=quote1quote ) [tabela] => Array ( [0] => domek ) [kolumna] => Array ( [0] => Nazwa [1] => Opis [2] => Cena ) [strona_wroc] => domki.php [komunikat_niepodzenie] => Ni chuja! [komunikat_powodzenie] => O, jest klucz! [typ_komendy] => 1 [submit] => Dodaj domek )
			
			
			
			//$w1 = "Nazwa=quote1quote";
			...
			//echo "<input type='hidden' name='warunek[]' value ='Insert' />"; //warunek where do funkcji baza
				
			echo "<input type='hidden' name='strona_wroc' value ='domki.php' />";
			echo "<input type='hidden' name='komunikat_niepodzenie' value ='Ni chuja!' />";
			echo "<input type='hidden' name='komunikat_powodzenie' value ='O, jest klucz!' />";
		...
		if (isset($_POST['edytuj'])) // jeśli kliknięto 'edytuj'
		{
			if (isset($_POST['lista'])) // jeśli wybrano rekord z formularza
			{
				...
				if ($row = mysql_fetch_row($edytowany_rekord))
				{
					...
					echo "Nazwa: <input type='text' name='wartosci[]' value=".$nazwa." size='15' /> <br/><br/>";
					echo "Cena: <input type='text' name='wartosci[]' value=".$cena." size='15' /> <br/><br/>";
					echo "Opis: <input type='text' name='wartosci[]' value=".$opis." size='15' /> <br/><br/>";
					...
					//$w1 = str_replace("quote","'",$w1);
					//$w2 = str_replace("quote","'",$w2);
					//$w3 = str_replace("quote","'",$w3);
					...
					echo "<input type='hidden' name='strona_wroc' value ='domki.php' />";
					echo "<input type='hidden' name='komunikat_niepowodzenie' value ='Dziewczyno, ty się ubierz!!' />";
					echo "<input type='hidden' name='komunikat_powodzenie' value ='Ale urwał! Ale to było dobre...!' />";
		...
		if (isset($_POST['usun'])) // jeśli kliknięto 'edytuj'
		{
			...
			//mysql_data_seek($usuwany_rekord, 0); // powrót na początek listy
			...
			echo "<input type='hidden' name='walidacja_warunek[]' value ='".$w2."' />";
			//echo "<input type='hidden' name='walidacja_warunek[]' value ='Tu jest 3 warunek walidacji' />";
					...
					//mysql_data_seek($usuwany_rekord, 0);
					...
					echo "<input type='hidden' name='strona_wroc' value ='domki.php' />";
					echo "<input type='hidden' name='komunikat_niepodzenie' value ='No chuj, nie wyszło!' />";
					echo "<input type='hidden' name='komunikat_powodzenie' value ='Yeah, ale z ciebie geniusz!' />";
