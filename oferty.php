...
if(isset($_SESSION['zalogowany'])) // jeśli zalogowany
{
	...
	if ($level == 6 || $level == 5 || $level == 3 || $level == 1 ) // sprawdź uprawnienia
	{
		if (isset($_POST['wyswietl'])) // jeśli kliknięto przycisk wyświetl
		{
			...
			if (isset($_POST['lista'])) // jeśli wybrano oferte
			{
				//echo $_POST['lista'];
				...
				$wybrany_rekord = mysql_query("SELECT * FROM oferta WHERE Nazwa = '$edytowane'"); // wybierz rekord do wyświetlenia
				$liczba_wierszy = @mysql_num_rows($wybrany_rekord); // przypisz zmiennej
				echo "<h1>Wybrana oferta</h1>";
				if ($liczba_wierszy > 0) //jeśli istnieją jakieś rekordy
				{
					...
					echo"<br/><br/>";
					echo"<br/><br/>";
					echo "<a href='oferty.php'>Wróć</a>";
					$lista = $wybrany_rekord;
				}
			}
			else{ //...a jeśli nie
				echo "Wybierz oferte  "; //wyświetl komunikat
				echo"<br/><br/>";
				echo"<br/><br/>";
				echo "<a href='oferty.php'>Wróć</a>";
			}
		...
		elseif (isset($_POST['wyszukaj'])) //jeśli klinięto wyszukaj
		{
			...
			if  ((empty($nazwa_szukanego_rekordu))) //jeśli pole nie zostało wypełnione
			{
				...
				echo "<a href='oferty.php'>Wróć</a>";
				header( 'refresh: 5; url=oferty.php' );
				$pagenum = 1;
			}
			else // ...a jeśli zostało
			{
				...
				mysql_select_db($db_name); //wybierz bazę
				// Else
				$wyszukane = mysql_query("SELECT * FROM oferta WHERE Nazwa LIKE '%$nazwa_szukanego_rekordu%' ORDER BY Nazwa ASC");
				...
				echo "<br><br><a href='oferty.php'>Wróć</a>";
			}
		}
		elseif ((!isset($_POST['dodaj'])) and (!isset($_POST['usun'])) and (!isset($_POST['edytuj'])))
		{
			...
			$wszystkie = mysql_query("SELECT * FROM oferta ORDER BY Nazwa ASC");
			//pejdżowanie
			...
			$data = "SELECT * FROM oferta ORDER BY Nazwa ASC LIMIT $na_stronie OFFSET $offset";
		...
		If (!empty($lista)) //jeśli zmienna $lista nie jest pusta
		{
			...
			echo"<option selected disabled>Wybierz oferte</option>";
		...
		if (isset($_POST['dodaj'])) // jeśli klinieto 'dodaj'
		{
			...
			echo "<input type='submit' name='dodaj' value='Dodaj oferte' />";
			echo "</form>";
			echo "<a href='oferty.php'>Wróć</a>";
			//$lista = $wybrany_rekord;
		}
		if (isset($_POST['edytuj'])) // jeśli kliknięto 'edytuj'
		{
			if (isset($_POST['lista'])) // jeśli wybrano rekord z formularza
			{
				...
				$connection=mysql_connect ($db_host, $db_user, $db_pass) or die ("Próba połączenie z bazą danych nie powiodła się. Spróbuj ponownie");
				mysql_select_db($db_name);
				$edytowany_rekord = mysql_query("SELECT * FROM oferta WHERE Nazwa = '$edytowane'");
				echo "<h1>Edytuj oferte</h1>";
				if ($row = mysql_fetch_row($edytowany_rekord))
				{
					...
					echo "<input type='submit' name='edytuj' value='Edytuj oferte' />";
					echo "</form>";
					echo "<a href='oferty.php'>Wróć</a>";
				...
				echo "Wybierz oferte  ";
				echo "</br></br></br><a href='oferty.php'>Wróć</a>";
			}
		}
		if (isset($_POST['usun'])) // jeśli kliknięto 'edytuj'
		{
			if (isset($_POST['lista'])) // jeśli wybrano rekord z formularza
			{
				...
				$connection=mysql_connect ($db_host, $db_user, $db_pass) or die ("Próba połączenie z bazą danych nie powiodła się. Spróbuj ponownie");
				mysql_select_db($db_name);
				$usuwany_rekord = mysql_query("SELECT * FROM oferta WHERE Nazwa = '$usuwane'");
				echo "<h1>Usunąć oferte z bazy?</h1>";
				if ($row = mysql_fetch_row($usuwany_rekord)) // jeśli wybrano rekord
				{
					...
					echo "<input type='submit' name='usun' value='Usuń oferte' />";
					echo "</form>";
					echo "<a href='oferty.php'>Wróć</a>";
